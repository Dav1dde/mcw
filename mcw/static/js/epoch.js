...
(function() {
  ...
  Epoch.Time.Plot = (function(superClass) {
    ...
    Plot.prototype.leftAxis = function() {
      var axis, domain, ticks;
      ticks = this.options.ticks.left;
      axis = d3.svg.axis().scale(this.ySvg()).orient('left').tickFormat(this.options.tickFormats.left);
      if (typeof ticks === "function" ? ticks() : void 0) {
        domain = this._getDomain();
        return axis.tickValues(ticks(domain[0], domain[1]));
      } else if (ticks === 2) {
        return axis.tickValues(this._getDomain());
      } else {
    ...
    Plot.prototype._getDomain = function() {
      var base, extent, range, ref;
      extent = this.extent(function(d) {
        return d.y;
      });
      range = (ref = typeof (base = this.options).range === "function" ? base.range(extent[0], extent[1]) : void 0) != null ? ref : this.options.range;
      return range != null ? range : extent;
    };

    Plot.prototype.y = function() {
      return d3.scale.linear().domain(this._getDomain()).range([this.innerHeight(), 0]);
    };

    Plot.prototype.ySvg = function() {
      return d3.scale.linear().domain(this._getDomain()).range([this.innerHeight() / this.pixelRatio, 0]);
    };
    ...
    Plot.prototype._updateTicks = function(newTime) {
      ...
      if (!(((ref = this._ticks[0]) != null ? ref.x : void 0) - (this.w() / this.pixelRatio) >= 0)) {
        return (ref1 = this._ticks[0]) != null ? ref1.exit = true : void 0;
      }
    };
